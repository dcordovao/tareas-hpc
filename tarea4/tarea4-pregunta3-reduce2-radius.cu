...
__global__ void reduce2(cell *g_idata, cell *g_odata)
{
...
int main(int argc, char *argv[]){
  ...
  // Search min load
  reduce2<<<large_out_1,blockSize,sharedBytes>>>(d_cells, dev_out);
  cudaDeviceSynchronize();
  cout << "first reduction" << endl;
  reduce2<<<large_out_2, blockSize, sharedBytes>>>(dev_out, dev_out2);
  cudaDeviceSynchronize();
  cout << "second reduction" << endl;
  reduce2<<<large_out_3,blockSize,sharedBytes>>>(dev_out2, dev_out3);
  cudaDeviceSynchronize();
  ...
  ofstream times_file;
  times_file.open("results_tarea_4_3_reduce2_radius.txt");
