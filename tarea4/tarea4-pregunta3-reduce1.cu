...
__global__ void reduce1(cell *g_idata, cell *g_odata)
{
    ...
    for (unsigned int s=1; s < blockDim.x; s *= 2) 
    {
        int index = 2 * s * tid;
        if (index < blockDim.x) {
            sdata[index] = (sdata[index].charge < sdata[index + s].charge)? sdata[index]: sdata[index + s];
        }
        __syncthreads();
    }
...
int main(int argc, char *argv[]){
  ...
  // Search min load
  reduce1<<<large_out_1,blockSize,sharedBytes>>>(d_cells, dev_out);
  cudaDeviceSynchronize();
  cout << "first reduction" << endl;
  reduce1<<<large_out_2, blockSize, sharedBytes>>>(dev_out, dev_out2);
  cudaDeviceSynchronize();
  cout << "second reduction" << endl;
  reduce1<<<large_out_3,blockSize,sharedBytes>>>(dev_out2, dev_out3);
  cudaDeviceSynchronize();
  ...
  ofstream times_file;
  times_file.open("results_tarea_4_3_reduce1.txt");
